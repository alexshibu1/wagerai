...
import {
  saveDeepWorkBlock,
  saveSessionVolatility,
  completeWager,
  failWager,
} from "@/app/actions/wager-actions";
...
export default function FocusSessionPage({
      ...
      if (storedTasks) {
        try {
          ...
          setTasks([
            ...
            {
              id: "2",
              title: "Draft outline for Q1 plan",
              completed: false,
              timebox: "deep",
            },
            {
              id: "3",
              title: "Review and revise budget",
              completed: false,
              timebox: "closing",
            },
          ]);
  ...
  const [deepWorkBlocks, setDeepWorkBlocks] = useState<DeepWorkBlock[]>(() => {
    if (typeof window !== "undefined" && params.id) {
      const stored = localStorage.getItem(
        `session_${params.id}_deepWorkBlocks`
      );
      if (stored) {
        try {
          ...
          return parsed.map((block: DeepWorkBlock & { savedAt?: number }) => {
            if (block.active && block.savedAt) {
              ...
              const newRemaining = Math.max(
                0,
                block.remainingSeconds - elapsed
              );
              if (newRemaining === 0) {
                return {
                  ...block,
  ...
  const [volatilityData, setVolatilityData] = useState<VolatilityDataPoint[]>(
    () => {
      if (typeof window !== "undefined" && params.id) {
        const stored = localStorage.getItem(
          `session_${params.id}_volatilityData`
        );
        if (stored) {
          try {
  ...
  const [currentFocusValue, setCurrentFocusValue] = useState(() => {
    if (typeof window !== "undefined" && params.id) {
      const stored = localStorage.getItem(
        `session_${params.id}_currentFocusValue`
      );
      if (stored) {
        try {
  ...
  const getStoredLastTaskCompletionTime = () => {
    if (typeof window !== "undefined" && params.id) {
      const stored = localStorage.getItem(
        `session_${params.id}_lastTaskCompletionTime`
      );
      if (stored && stored !== "null") {
        try {
          ...
          console.error(
            "Failed to parse stored last task completion time:",
            err
          );
        }
      }
  ...
  const lastTaskCompletionTime = useRef<number | null>(
    getStoredLastTaskCompletionTime()
  );
  const taskCompletionCount = useRef(0);
  const [momentumWarning, setMomentumWarning] = useState<{
    seconds: number;
  ...
  const lastVisibleDecayTime = useRef<number>(
    storedLastTaskTime || sessionStartTime.current
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `${storageKey}_deepWorkBlocks`,
      JSON.stringify(blocksToSave)
    );

    // Save volatility data (keep last 100 points)
    localStorage.setItem(
      `${storageKey}_volatilityData`,
      JSON.stringify(volatilityData.slice(-100))
    );

    // Save current focus value
    localStorage.setItem(
      `${storageKey}_currentFocusValue`,
      currentFocusValue.toString()
    );

    // Save session start time
    localStorage.setItem(
      `${storageKey}_startTime`,
    ...
    if (lastTaskCompletionTime.current) {
      localStorage.setItem(
        `${storageKey}_lastTaskCompletionTime`,
        lastTaskCompletionTime.current.toString()
      );
    } else {
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      if (hasActiveBlock || currentFocusValue > 100) {
        e.preventDefault();
        e.returnValue =
          "You have an active deep work session. Your focus value will slowly drain if you leave. Are you sure you want to leave?";
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const visibilityDrainMultiplier =
        !isPageVisible && !hasMultipleScreens ? 1.5 : 1.0;
      ...
      const deepWorkIncrease = hasActiveDeepWork
        ? activeBlocks.length * DEEP_WORK_INCREASE_RATE
        : 0;
        ...
        if (hasActiveDeepWork) {
          ...
          setCurrentFocusValue((prev) => {
            ...
            setVolatilityData((prevData) => {
              ...
              if (shouldAddPoint) {
                const newData = [
                  ...prevData,
                  { time: timeLabel, value: newValue, event: "deepwork" },
                ];
          ...
          setCurrentFocusValue((prev) => {
            if (prev > MIN_VALUE) {
              ...
              const timeSinceLastDecay = Math.floor(
                (now - lastVisibleDecayTime.current) / 1000
              );
              ...
              if (timeSinceLastDecay >= VISIBLE_DECAY_INTERVAL) {
                ...
                const intervalsPassed = Math.floor(
                  timeSinceLastDecay / VISIBLE_DECAY_INTERVAL
                );
                ...
                lastVisibleDecayTime.current =
                  now - (timeSinceLastDecay % VISIBLE_DECAY_INTERVAL) * 1000;
              }
              ...
              setVolatilityData((prevData) => {
                const lastPoint = prevData[prevData.length - 1];
                const shouldAddPoint =
                  decayHappened || // Always add point when decay happens
                  !lastPoint ||
                  Math.abs(newValue - lastPoint.value) >= 0.3 ||
                  updateCounter.current % 2 === 0;
      ...
      setCurrentFocusValue((prev) => {
        ...
        if (secondsSinceLastTask !== null && secondsSinceLastTask < 300) {
          ...
          newValue = Math.max(
            MIN_VALUE,
            Math.min(MAX_VALUE, prev + totalChange)
          );
        } else {
          ...
          if (block.active && block.remainingSeconds > 0) {
            ...
            if (wager?.id) {
              ...
              saveDeepWorkBlock(
                ...
                console.error("Failed to save deep work block:", err)
              );
            }
        ...
        if (task.id === taskId) {
          ...
          if (newCompleted) {
            ...
            const elapsedHours = Math.floor(
              (now - sessionStartTime.current) / (1000 * 60 * 60)
            );
            ...
            const remainingCompleted = prev.filter(
  ...
  const handleStartDeepWork = (blockId: number) => {
    const availableBlock = deepWorkBlocks.find(
      (b) => b.id === blockId && !b.active && !b.completed
    );
  ...
  const handleCancelDeepWork = (blockId: number) => {
    if (
      !confirm("Cancel deep work? This will cause a massive volatility drop!")
    )
      return;
    ...
    setDeepWorkBlocks((prev) =>
      prev.map((block) =>
        block.id === blockId
          ? { ...block, active: false, remainingSeconds: 0, taskId: undefined }
          : block
      )
    );
    ...
    setCurrentFocusValue((prevValue) => {
      ...
      setVolatilityData((prevData) => {
        const newData = [
          ...prevData,
          { time: timeLabel, value: drop, event: "cancel" },
        ];
        return newData.slice(-100);
      });
      return drop;
  ...
  const handleCompleteDay = async () => {
    ...
    try {
      ...
      alert(
        `Failed to complete day: ${errorMessage}\n\nPlease check the console for more details.`
      );
  ...
  const handleForfeit = () => {
    if (
      confirm(
        "Are you sure you want to forfeit this wager? You will lose $" +
          (wager?.stake_amount || 50)
      )
    ) {
      router.push("/markets");
    }
  ...
  return (
    <main
      className={`w-full min-h-screen ${bgClass} relative transition-all duration-1000 ${isEntering ? "opacity-0" : "opacity-100"}`}
    >
        ...
        <div className="mb-8 max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            <div className="text-center mb-4">
              <h1
                className="text-2xl font-medium text-zinc-300 mb-1 tracking-wide"
                style={{ fontFamily: '"Space Grotesk", sans-serif' }}
              >
                Welcome {userName}
              </h1>
              <p className="text-sm text-cyan-400/70 mb-6 uppercase tracking-[0.3em] font-medium">
                Deep Work Terminal
              </p>
              <div className="relative inline-block">
                <div
                  ...
                  style={{
                    fontFamily:
                      '"SF Pro Display", "SF Pro Text", -apple-system, BlinkMacSystemFont, "Segoe UI", "Inter", "Poppins", sans-serif',
                    ...
                    {deepWorkBlocks
                      ...
                      .map((block) => {
                        ...
                        const minutes = Math.floor(
                          (block.remainingSeconds % 3600) / 60
                        );
                        ...
                        return (
                          <div
                            ...
                            style={{
                              background:
                                "linear-gradient(165deg, rgba(6, 182, 212, 0.12) 0%, rgba(10, 10, 15, 0.95) 100%)",
                          ...
                          >
                            <div className="flex items-center gap-4 w-full min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div
                                  className="text-lg font-mono font-medium text-white leading-tight mb-0.5"
                    ...
                    >
                      <div className="flex items-center gap-3 w-full min-w-0">
                        ...
                        <div className="flex-1 min-w-0 text-left">
                          <div
                            className={`text-sm font-semibold truncate mb-0.5 leading-tight ${isActive ? colors.text : "text-white"}`}
                          >
                            {session.title.startsWith("Daily Session:")
                              ? "Daily Session"
                              : session.title}
                          </div>
                          <div className="flex items-center gap-2 text-xs font-mono leading-tight">
                            ...
                            >
                              {String(session.timeRemaining.hours).padStart(
                                2,
                                "0"
                              )}
                              :
                              {String(session.timeRemaining.minutes).padStart(
                                2,
                                "0"
                              )}
                              :
                              {String(session.timeRemaining.seconds).padStart(
                                2,
                                "0"
                              )}
                            </span>
                        ...
                        <>
                          <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                            <span className="text-emerald-400 font-mono text-lg font-medium tracking-tight">
                              ...
                              {String(block.remainingSeconds % 60).padStart(
                                2,
                                "0"
                              )}
                            </span>
            ...
            <div className="mt-8">
              <Button
                ...
                style={{
                  ...
                  boxShadow:
                    "0 0 30px rgba(185, 28, 28, 0.3), 0 0 50px rgba(185, 28, 28, 0.2)",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow =
                    "0 0 40px rgba(185, 28, 28, 0.5), 0 0 70px rgba(185, 28, 28, 0.3)";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow =
                    "0 0 30px rgba(185, 28, 28, 0.3), 0 0 50px rgba(185, 28, 28, 0.2)";
          ...
          <div className="flex flex-col">
            ...
            >
              <div className="p-4 pb-2">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-lg font-mono font-bold ${currentFocusValue >= 100 ? "text-emerald-400" : "text-red-400"}`}
                    >
              ...
              <div className="h-[250px] px-4 pb-4">
                <VolatilityChart
                  data={volatilityData}
                  currentValue={currentFocusValue}
                />
              </div>
            </div>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-400">
                    {Math.floor(momentumWarning.seconds / 60)}m{" "}
                    {momentumWarning.seconds % 60}s
                    {lastTaskCompletionTime.current
                      ? " since last task"
                      : " elapsed"}
                  </span>
